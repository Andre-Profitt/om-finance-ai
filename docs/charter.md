...
**Live numbers from the canonical run live in [`docs/current-results.md`](current-results.md)** — regenerated by `make demo`. This table specifies durable acceptance bars, not specific run snapshots.

| Metric                                                                               | Acceptance bar                                                          |
| ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- |
| Calibrated precision@top-100 on reviewer queue                                       | ≥ 0.85                                                                  |
| Dollars-at-risk captured per reviewer hour                                           | ≥ 3× unassisted baseline                                                |
| Citation precision on RAG explanations                                               | ≥ 0.90                                                                  |
| Abstention rate                                                                      | within [10%, 40%]                                                       |
| Override-with-rationale rate                                                         | 100% of decisions carry a rationale                                     |
| Payback period (modeled, base case; bear case in `docs/roi-model.md` is negative Y1) | ≤ 9 months on a mid-size practice for the base case                     |
| Model lineage completeness                                                           | 100% of scored items traceable to model + prompt + source-doc version   |
| Drift alarm coverage                                                                 | Calibration, citation precision, abstention, override rate each alarmed |
...
4. Multispecialty generalizes from oncology primitives because the underlying mechanics (payer, drug, rebate, access) repeat — **validated in V1 by running the same pipeline across 5 specialties at the citation precision bar in `docs/current-results.md`**
