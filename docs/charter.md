...
McKesson's Oncology & Multispecialty (O&M) segment is a connected specialty-care ecosystem: US Oncology Network practices, Ontada, Biologics, CoverMyMeds, Onmark GPO, iKnowMed, Glide Health, and multispecialty providers in retinal, rheumatology, gastroenterology, and neurology care. Each capability produces financial and operational signal. This artifact models a candidate finance-side operating layer that could sit above those signals: a governed, evidence-grounded reviewer queue for prioritization, ROI sizing, controllership review, and adoption measurement. Whether O&M Finance has an analogous internal capability today is for the team to confirm — the artifact is meant as a concrete starting point, not a claim about the current state.
