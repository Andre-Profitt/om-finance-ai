...
| Gold: rule-based exception candidates across 10 priority-ordered types                                                                                                                        | Shipped — `access_pa_gap`, `jw_drug_waste`, `gpo_340b_rebate_excluded`, `chargeback_validity_fail`, `biosimilar_conversion_miss`, `site_of_care_underpayment`, `ndc_hcpcs_mismatch`, `asp_drift`, `underpayment`, `denial` |
| LightGBM risk scorer + MLflow registry                                                                                                                                                        | Shipped                                                                                                                                                                                                                    |
| 5-fold CV isotonic calibration with sample-weighted reliability slope                                                                                                                         | Shipped                                                                                                                                                                                                                    |
...
| 9 Databricks SQL queries + 6 drift alarms + Unity Catalog RLS DDL                                                                                                                             | Shipped                                                                                                                                                                                                                    |
| Charter, PRD, ROI model (with bear case), decision log, governance ref arch, current-results, discovery plan, value-realization runbook, RACI, acquisition scorecard, model card, demo script | Shipped                                                                                                                                                                                                                    |

**V1.1 acceptance bars (all hit on the canonical run; live numbers in [`docs/current-results.md`](current-results.md)):**

- LightGBM valid AUC ≥ 0.85 (debug-only metric, never the headline)
- Calibrated precision@top-100 ≥ 0.85
- Calibration slope within [0.85, 1.15] after 5-fold CV isotonic with sample-weighted reliability fit
- Citation precision ≥ 0.90 on explained exceptions
...
| Production calibration at scale (5-fold CV isotonic on weekly refresh)                                         | V1 uses in-session 5-fold CV on one batch; V2 runs the same on the rolling weekly window                                   |
...
| Per-payer policy expansion (10+ commercial + Medicare LCDs)                                                    | V1 has 10 payer policies in corpus; V2 expands to production payer-specific LCD/commercial policy coverage                 |
| Per-vendor GPO contract expansion and chargeback flow end-to-end                                               | V1 has 6 mock GPO contracts; V2 expands to production vendor-specific contract and chargeback flows                        |
